...
# Blueprint - MANTENEMOS LA NOMENCLATURA ORIGINAL
diagramaClases_bp = Blueprint('generador_diagramas_clases', __name__)
...
llm = AzureChatOpenAI(
    ...
    openai_api_key=os.getenv("AZURE_OPENAI_API_KEY", "6pjnaxBV5PuLJSuipSo8in4PBxK4KC2r6RQ76isgZBubY7gqFqZuJQQJ99BFACYeBjFXJ3w3AAABACOGhPN0"),
...
# Template para diagramas de clases
class_diagram_prompt = PromptTemplate(
    input_variables=["contenido_archivo", "contexto_adicional"],
    template="""
Eres un arquitecto de software experto en UML. Genera un diagrama de clases completo en formato XML para draw.io.
...
2. Identifica todas las clases, interfaces, enums y relaciones
3. Incluye atributos con tipos de datos y visibilidad (+, -, #, ~)
4. Incluye métodos con parámetros, tipos de retorno y visibilidad
5. Representa relaciones: herencia, implementación, composición, agregación, asociación
6. Usa estereotipos cuando sea apropiado (<<interface>>, <<abstract>>, <<entity>>)
7. Posiciona las clases de manera organizada y legible
...
@diagramaClases_bp.route('/generate', methods=['POST'])
def generate_class_diagram():
    """Genera diagrama de clases en formato draw.io"""
    try:
        ...
        return jsonify({
            ...
            'diagram_type': 'class',
            'message': 'Diagrama de clases generado exitosamente',
...
@diagramaClases_bp.route('/health', methods=['GET'])
def health_check():
    """Endpoint de salud para el servicio"""
    return jsonify({
        'status': 'healthy',
        'service': 'class_diagram',
        'timestamp': datetime.now().isoformat()
    })
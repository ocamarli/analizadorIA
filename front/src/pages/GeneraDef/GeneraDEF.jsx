...
const GeneraDefRequerimientos = () => {
  ...
  const generateDef = async () => {
    ...
    try {
      ...
      const apiResponse = await fetch(`${API_BASE_URL}/api/generar-def-requerimientos`, {
        method: 'POST',
        body: formData,
      });
  ...
  const downloadWord = () => {
    ...
    const wordContent = `
      ...
      <head>
        <meta charset="utf-8">
        <title>DEF - Definición de Requerimientos Funcionales</title>
        <style>
          body { 
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = `DEF_Requerimientos_${new Date().toISOString().split('T')[0]}.doc`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
  ...
  return (
    <Box sx={{ p: 3, maxWidth: 800, margin: '0 auto' }}>
      <Typography variant="h4" gutterBottom align="center">
        Generador DEF - Definición de Requerimientos Funcionales
      </Typography>
...
export default GeneraDefRequerimientos;
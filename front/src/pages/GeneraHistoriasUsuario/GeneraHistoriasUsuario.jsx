...
const GeneradorHistoriasUsuario = () => {
  ...
  const generateUserStories = async () => {
    ...
    if (files.length === 0 && !additionalText.trim()) {
      showNotification('Selecciona archivos o ingresa información de roles', 'warning');
      return;
    }
    ...
    try {
      ...
      const apiResponse = await fetch(`${API_BASE_URL}/api/generar-historias-usuario`, {
        method: 'POST',
        body: formData,
      });
      ...
      showNotification('¡Historias de usuario generadas!', 'success');
  ...
  const downloadMarkdown = () => {
    ...
    link.href = url;
    link.download = `Historias_Usuario_${new Date().toISOString().split('T')[0]}.md`;
  ...
  const downloadWord = () => {
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = `Historias_Usuario_${new Date().toISOString().split('T')[0]}.doc`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
    ...
    <Box sx={{ p: 3, maxWidth: 800, margin: '0 auto' }}>
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12}>
          <Paper sx={{ p: 3 }}>
            <Typography variant="h6" gutterBottom>
              Documentos de Requerimientos y Análisis
            </Typography>
        ...
        <Grid item xs={12}>
          <Paper sx={{ p: 3 }}>
            <Typography variant="h6" gutterBottom>
              Información de Roles y Usuarios (Opcional)
            </Typography>
            <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
              Define roles de usuario, personas, workflows específicos y contexto adicional para generar historias más precisas.
            </Typography>
            ...
            <TextField
              ...
              placeholder="Ejemplo:
ROLES DE USUARIO:
- Administrador: Gestiona configuraciones y usuarios del sistema
- Operador: Realiza operaciones diarias de inventario
- Supervisor: Revisa y aprueba procesos críticos
- Auditor: Consulta reportes y realiza auditorías

PERSONAS:
- Juan (Operador): 35 años, usa el sistema 8h/día, necesita procesos rápidos
- María (Supervisora): 45 años, revisa reportes semanales, prioriza precisión

WORKFLOWS ESPECÍFICOS:
- Proceso de inventario mensual con validación biométrica
- Generación de reportes automáticos cada viernes
- Flujo de aprobación con notificaciones por email

CONTEXTO DEL NEGOCIO:
- Empresa de retail con 50 sucursales
- Inventarios críticos para la operación
- Integración con sistema ERP existente..."
        ...
        <Grid item xs={12}>
          <Button
            ...
            sx={{ py: 1.5 }}
          >
            {isProcessing ? 'Generando...' : 'Generar Historias de Usuario'}
          </Button>
        </Grid>
...
export default GeneradorHistoriasUsuario;
...
const GeneradorDocumentoArquitectura = () => {
  ...
  const generateArchitectureDocument = async () => {
    ...
    try {
      ...
      const apiResponse = await fetch(`${API_BASE_URL}/api/generar-documento-arquitectura`, {
        method: 'POST',
        body: formData,
      });
      ...
      showNotification('¡Documento de arquitectura generado!', 'success');
  ...
  const downloadMarkdown = () => {
    ...
    link.href = url;
    link.download = `Documento_Arquitectura_${new Date().toISOString().split('T')[0]}.md`;
  ...
  const downloadWord = () => {
    ...
    // Plantilla HTML para Word con estilos para documentos de arquitectura
    const wordContent = `
      <!DOCTYPE html>
      <html>
      <head>
        <meta charset="utf-8">
        <title>Documento de Arquitectura de Software</title>
        <style>
          body { 
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = `Documento_Arquitectura_${new Date().toISOString().split('T')[0]}.doc`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
  ...
  return (
    <Box sx={{ p: 3, maxWidth: 800, margin: '0 auto' }}>
      <Typography variant="h4" gutterBottom align="center">
        Generador de Documentos de Arquitectura de Software
      </Typography>
        ...
        <Grid item xs={12}>
          <Paper sx={{ p: 3 }}>
            ...
            <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
              Puedes agregar contexto técnico específico, patrones de arquitectura, tecnologías, o cualquier información que complemente los documentos subidos.
            </Typography>
            ...
            <TextField
              ...
              variant="outlined"
              placeholder="Ejemplo: 
- Sistema de microservicios con Spring Boot y Java 21
- Frontend Angular 17 con arquitectura de componentes
- Base de datos PostgreSQL con replicación master-slave
- API Gateway Kong para enrutamiento y autenticación JWT
...
- Integraciones: API externa de pagos, servicio de notificaciones..."
        ...
        <Grid item xs={12}>
          <Button
            ...
            size="large"
            fullWidth
            onClick={generateArchitectureDocument}
            ...
            sx={{ py: 1.5 }}
          >
            {isProcessing ? 'Generando...' : 'Generar Documento de Arquitectura'}
          </Button>
        </Grid>
...
export default GeneradorDocumentoArquitectura;
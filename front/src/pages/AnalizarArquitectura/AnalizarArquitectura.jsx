...
const AnalizadorArquitectura = () => {
  ...
  const handleFileChange = (event) => {
    ...
    if (validFiles.length !== selectedFiles.length) {
      showNotification('Solo archivos PDF, Markdown, XML/DrawIO y Mermaid', 'warning');
    }
  ...
  const analyzeArchitecture = async () => {
    ...
    try {
      ...
      const apiResponse = await fetch(`${API_BASE_URL}/api/analizar-arquitectura`, {
        method: 'POST',
        body: formData,
      });
      ...
      showNotification('¡Análisis de arquitectura completado!', 'success');
  ...
  const downloadMarkdown = () => {
    ...
    link.href = url;
    link.download = `Analisis_Arquitectura_${new Date().toISOString().split('T')[0]}.md`;
  ...
  const downloadWord = () => {
    ...
    const wordContent = `
      ...
      <head>
        <meta charset="utf-8">
        <title>Análisis de Arquitectura de Software</title>
        <style>
          body { 
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = `Analisis_Arquitectura_${new Date().toISOString().split('T')[0]}.doc`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
  ...
  return (
    <Box sx={{ p: 3, maxWidth: 800, margin: '0 auto' }}>
      <Typography variant="h4" gutterBottom align="center">
        Analizador de Arquitectura de Software
      </Typography>
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12}>
          <Paper sx={{ p: 3 }}>
            <Typography variant="h6" gutterBottom>
              Documentos y Diagramas de Arquitectura
            </Typography>
            ...
            >
              <input
                ...
                style={{ display: 'none' }}
                multiple
                accept=".pdf,.md,.markdown,.xml,.drawio,.mmd"
              />
              ...
              <Typography variant="body2" color="text.secondary">
                Formatos: PDF, Markdown (.md), XML/DrawIO, Mermaid (.mmd)
              </Typography>
            </Paper>
        ...
        <Grid item xs={12}>
          <Paper sx={{ p: 3 }}>
            <Typography variant="h6" gutterBottom>
              Contexto Adicional de Arquitectura (Opcional)
            </Typography>
            <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
              Puedes agregar contexto sobre la arquitectura: tecnologías específicas, patrones, requisitos no funcionales, integraciones, etc.
            </Typography>
...
- Requisitos: 10k usuarios concurrentes, 99.9% uptime
- Integraciones: APIs REST, message queues
- Patrones: CQRS, Event Sourcing..."
        ...
        <Grid item xs={12}>
          <Button
            ...
            sx={{ py: 1.5 }}
          >
            {isProcessing ? 'Analizando...' : 'Analizar Arquitectura de Software'}
          </Button>
        </Grid>
            ...
            <Paper sx={{ p: 3 }}>
              <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
                <Typography variant="h6">
                  Análisis de Arquitectura Generado
                </Typography>
...
export default AnalizadorArquitectura;
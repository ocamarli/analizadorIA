...
const GeneradorArquitecturaToBe = () => {
  ...
  // Generar arquitectura TO BE
  const generateToBe = async () => {
    // Validar que haya al menos archivos o texto
    if (files.length === 0 && !additionalText.trim()) {
      showNotification('Selecciona archivos o ingresa especificaciones de migración', 'warning');
      return;
    }
    ...
    try {
      ...
      const apiResponse = await fetch(`${API_BASE_URL}/api/generar-arquitectura-tobe`, {
        method: 'POST',
        body: formData,
      });
      ...
      showNotification('¡Arquitectura TO BE generada!', 'success');
  ...
  const downloadMarkdown = () => {
    ...
    link.href = url;
    link.download = `Arquitectura_TO_BE_${new Date().toISOString().split('T')[0]}.md`;
  ...
  const downloadWord = () => {
    ...
    const wordContent = `
      ...
      <head>
        <meta charset="utf-8">
        <title>Arquitectura TO BE - Diseño de Arquitectura Objetivo</title>
        <style>
          body { 
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = `Arquitectura_TO_BE_${new Date().toISOString().split('T')[0]}.doc`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
  ...
  return (
    <Box sx={{ p: 3, maxWidth: 800, margin: '0 auto' }}>
      <Typography variant="h4" gutterBottom align="center">
        Generador de Arquitectura TO BE
      </Typography>
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12}>
          <Paper sx={{ p: 3 }}>
            <Typography variant="h6" gutterBottom>
              Documentos AS IS y Análisis Existente
            </Typography>
        ...
        <Grid item xs={12}>
          <Paper sx={{ p: 3 }}>
            <Typography variant="h6" gutterBottom>
              Especificaciones de Migración y Tecnologías TO BE
            </Typography>
            <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
              Define las tecnologías objetivo, patrones arquitectónicos, y especificaciones para la nueva arquitectura TO BE.
            </Typography>
...
- Seguridad: OAuth 2.0 + JWT, encriptación E2E..."
        ...
        <Grid item xs={12}>
          <Button
            ...
            sx={{ py: 1.5 }}
          >
            {isProcessing ? 'Generando...' : 'Generar Arquitectura TO BE'}
          </Button>
        </Grid>
...
export default GeneradorArquitecturaToBe;
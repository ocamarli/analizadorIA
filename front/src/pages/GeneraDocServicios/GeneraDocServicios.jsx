...
const GeneradorDocumentoServicio = () => {
  ...
  // Generar documento de servicio
  const generateServiceDocument = async () => {
    // Validar que haya al menos archivos o texto
    if (files.length === 0 && !additionalText.trim()) {
      showNotification('Selecciona archivos o ingresa información técnica', 'warning');
      return;
    }
    ...
    try {
      ...
      const apiResponse = await fetch(`${API_BASE_URL}/api/generar-documento-servicio`, {
        method: 'POST',
        body: formData,
      });
      ...
      showNotification('¡Documento de servicio generado!', 'success');
  ...
  const downloadMarkdown = () => {
    ...
    link.href = url;
    link.download = `Documento_Servicio_${new Date().toISOString().split('T')[0]}.md`;
  ...
  const downloadWord = () => {
    ...
    // Plantilla HTML para Word con estilos para documentos técnicos de servicios
    const wordContent = `
      <!DOCTYPE html>
      <html>
      <head>
        <meta charset="utf-8">
        <title>Documento Técnico de Servicio</title>
        <style>
          body { 
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = `Documento_Servicio_${new Date().toISOString().split('T')[0]}.doc`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
  ...
  return (
    <Box sx={{ p: 3, maxWidth: 800, margin: '0 auto' }}>
      <Typography variant="h4" gutterBottom align="center">
        Generador de Documentos Técnicos de Servicios/APIs
      </Typography>
        ...
        <Grid item xs={12}>
          <Paper sx={{ p: 3 }}>
            <Typography variant="h6" gutterBottom>
              Documentos de Especificación del Servicio
            </Typography>
        ...
        <Grid item xs={12}>
          <Paper sx={{ p: 3 }}>
            ...
            <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
              Puedes agregar detalles técnicos específicos del servicio, endpoints, configuraciones, dependencias, o cualquier información que complemente los documentos subidos.
            </Typography>
            ...
            <TextField
              ...
              variant="outlined"
              placeholder="Ejemplo: 
- Servicio REST API desarrollado en Java 21 con Spring Boot 3.3.2
- Base de datos PostgreSQL v15 con conexión mediante JPA
- Expone endpoints para gestión de catálogos de muebles de exhibición
- Operaciones CRUD: consulta, alta, baja, modificación y validación
- Autenticación mediante JWT y autorización basada en roles
- Timeouts configurados entre 100ms y 3000ms según endpoint
- Deployed en Kubernetes con health checks y readiness probes
- Logging estructurado con trazabilidad de transacciones
...
- Rate limiting implementado en API Gateway..."
        ...
        <Grid item xs={12}>
          <Button
            ...
            size="large"
            fullWidth
            onClick={generateServiceDocument}
            ...
            sx={{ py: 1.5 }}
          >
            {isProcessing ? 'Generando...' : 'Generar Documento de Servicio'}
          </Button>
        </Grid>
            ...
            <Paper sx={{ p: 3 }}>
              <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
                <Typography variant="h6">
                  Documento de Servicio Generado
                </Typography>
...
export default GeneradorDocumentoServicio;
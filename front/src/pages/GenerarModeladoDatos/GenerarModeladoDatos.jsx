...
const GeneradorModeladoDatos = () => {
  ...
  const generateDataModeling = async () => {
    ...
    try {
      ...
      const apiResponse = await fetch(`${API_BASE_URL}/api/generar-modelado-datos`, {
        method: 'POST',
        body: formData,
      });
      ...
      showNotification('¡Modelado de datos generado!', 'success');
  ...
  const downloadMarkdown = () => {
    ...
    link.href = url;
    link.download = `Modelado_Datos_${new Date().toISOString().split('T')[0]}.md`;
  ...
  const downloadWord = () => {
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = `Modelado_Datos_${new Date().toISOString().split('T')[0]}.doc`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
  ...
  return (
    <Box sx={{ p: 3, maxWidth: 800, margin: '0 auto' }}>
      <Typography variant="h4" gutterBottom align="center">
        Generador de Modelado de Datos
      </Typography>
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12}>
          <Paper sx={{ p: 3 }}>
            <Typography variant="h6" gutterBottom>
              Documentos de Análisis y Historias Técnicas
            </Typography>
        ...
        <Grid item xs={12}>
          <Paper sx={{ p: 3 }}>
            <Typography variant="h6" gutterBottom>
              Especificaciones de Base de Datos (Opcional)
            </Typography>
            <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
              Define detalles específicos del motor de BD, convenciones de nomenclatura, políticas de datos y configuraciones especiales.
            </Typography>
...
- Tablas: PascalCase (ej: UserAccount, OrderItem)
- Campos: camelCase (ej: userId, firstName, createdAt)
- Índices: idx_tabla_campo (ej: idx_user_email)
- Foreign Keys: FK_tabla1_tabla2 (ej: FK_Order_User)

POLÍTICAS DE DATOS:
- Soft deletes con campo 'deletedAt'
- Timestamps automáticos (createdAt, updatedAt)
- Campos obligatorios: id (UUID), createdAt, updatedAt
- Cifrado para campos sensibles (PII, financial data)

CONFIGURACIONES ESPECÍFICAS:
- Backup automático diario a las 2:00 AM
- Retención de logs: 30 días
- Connection pooling: máximo 100 conexiones
- Query timeout: 30 segundos

DATOS SENSIBLES:
- Información biométrica: SÍ (tabla UserBiometrics)
- Datos financieros: SÍ (tabla PaymentInfo)
- PII: SÍ (campos email, phone, address)

ÍNDICES ESPECIALES:
- Full-text search en campos de descripción
- Índices compuestos para consultas frecuentes
- Particionado por fecha en tablas de logs..."
        ...
        <Grid item xs={12}>
          <Button
            ...
            sx={{ py: 1.5 }}
          >
            {isProcessing ? 'Generando...' : 'Generar Modelado de Datos'}
          </Button>
        </Grid>
...
export default GeneradorModeladoDatos;